...
def _ref_full(word, tries=6):
    import os, subprocess
    root = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), ".."))
    cli = os.path.join(root, "transcr_cli.exe")
    best = None
    for _ in range(tries):
        out = subprocess.run([cli, "--full", word], capture_output=True).stdout
        txt = out.decode("cp1257", "replace")
        try:
            dur_blk = txt.split("@@@")[1].split("%%%")[0]
            ton_blk = txt.split("%%%")[1].split("###")[0]
        except IndexError:
            continue
        durs = [l.strip() for l in dur_blk.splitlines() if l.strip()]
        tons = [l.strip() for l in ton_blk.splitlines() if l.strip()]
        if len(durs) > 2:
            return durs, tons
        best = (durs, tons)
    return best
...
if __name__ == "__main__":
    import sys
    words = sys.argv[1:] or ["rytas", "vakar", "sveiki", "obuolys", "labas rytas"]
    for w in words:
        durs, ref = _ref_full(w)
        mine = _fmt(tonai(durs))
        ok = mine == ref
        print("%s %s" % ("OK " if ok else "XX", w))
        if not ok:
            for a, b in zip(ref, mine):
                flag = "  " if a == b else ">>"
                print("   %s ref=%-26s mine=%s" % (flag, a, b))
